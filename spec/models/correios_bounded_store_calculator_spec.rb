...
describe CorreiosBoundedStoreCalculator do
  ...
  before(:each) do
    ...
    @soap_service = Spree::CorreiosBoundedStore::BogusSoapService.new
    @calculator = Spree::CorreiosBoundedStore::BogusCalculator.new
    @calculator.stub!(:soap_service).and_return(@soap_Service)
  end

  describe "calculate_shipping" do
    it "should use the soap service supplied in the initializer" do
      @soap_service.should_receive(:consultarFrete).and_return(Spree::CorreiosBoundedStore::DefinedRateResponse.new(true, "Foo"))
